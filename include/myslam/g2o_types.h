...
public:
    ...
    // Update with left multiplication on SE3 with array
...
// Unary edge that only concern pose
...
private:
    Vec3 _pos3d;            // what is this pose ?
    Mat33 _K;               // Camera intrinsics

};


// Binary edge with landmarks and pose
...
public:
    ...
    virtual void computeError() override {
        ...
        SE3 T = v0->estimate();
        Vec3 pw = v1->estimate();
        Vec3 pos_pixel = K * (_cam_ext * (T * pw));
        pos_pixel /= pos_pixel[2];
        _error = _measurement - pos_pixel.head<2>();
    }
    ...
    virtual void linearizeOplus() override {
        ...
        SE3 T = v0->estimate();
        Vec3 pw = v1->estimate();
        Vec3 pos_cam = _cam_ext * T * pw;           // landmark position at camera coordinate
